...
class MonitoringManager {
    ...
    func prepareThresholds(selection: FamilyActivitySelection, limitMinutes: Int) async {
        ...
        let events = await Task.detached(priority: .userInitiated) { [selection, limitMinutes] in
            ...
            // Note: buildEvents needs to be called on MainActor, so we'll do it here
            // Actually, we can't call buildEvents from detached task because it needs MainActor
            // So we'll build events directly here
    ...
    func startMonitoring(selection: FamilyActivitySelection, limitMinutes: Int) async {
        ...
        do {
            ...
            NSLog("MARKERS MonitoringManager: ✅ Successfully started monitoring with %d events", events.count)
            fflush(stdout)
        } catch {
