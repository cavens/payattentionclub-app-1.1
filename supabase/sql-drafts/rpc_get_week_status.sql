...
begin
  ...
  user_settlement_status :=
    coalesce(v_user_week_pen.settlement_status, 'pending');

  charged_amount_cents :=
    coalesce(v_user_week_pen.charged_amount_cents, 0);

  actual_amount_cents :=
    coalesce(v_user_week_pen.actual_amount_cents, v_user_week_pen.total_penalty_cents, 0);

  refund_amount_cents :=
    coalesce(v_user_week_pen.refund_amount_cents, 0);

  needs_reconciliation :=
    coalesce(v_user_week_pen.needs_reconciliation, false);

  reconciliation_delta_cents :=
    coalesce(v_user_week_pen.reconciliation_delta_cents, 0);

  reconciliation_reason :=
    v_user_week_pen.reconciliation_reason;

  reconciliation_detected_at :=
    v_user_week_pen.reconciliation_detected_at;

  week_grace_expires_at :=
    coalesce(v_commitment.week_grace_expires_at, v_week_start_date + interval '1 day');
