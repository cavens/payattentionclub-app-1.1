...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (eventType === "payment_intent.succeeded") {
      ...
      const { data: paymentRow, error: paymentErr } = await supabase.from("payments").select("id, user_id, week_start_date").eq("stripe_payment_intent_id", paymentIntentId).maybeSingle();
      if (paymentErr) {
      ...
      if (!paymentRow) {
        console.warn("No matching payment row found for payment_intent:", paymentIntentId);
        return new Response("OK", {
          status: 200
        });
      }
      // Update payment status
      const { error: updatePaymentErr } = await supabase.from("payments").update({
        status: "succeeded"
      }).eq("id", paymentRow.id);
      if (updatePaymentErr) {
      ...
      const { error: updatePenaltyErr } = await supabase.from("user_week_penalties").update({
        status: "paid"
      }).eq("user_id", paymentRow.user_id).eq("week_start_date", paymentRow.week_start_date);
      if (updatePenaltyErr) {
      ...
      if (!paymentRow) {
        console.warn("No matching payment row found for failed payment_intent:", paymentIntentId);
        return new Response("OK", {
          status: 200
        });
      }
