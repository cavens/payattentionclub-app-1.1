...
Deno.serve(async (req)=>{
  try {
    ...
    // 2.5) Handle "no sync" case: Active commitments with no daily_usage entries
    // Phase 6: Settlement Rule Option B (worst-case) - Assume max overuse if no sync
    // Fetch all active (non-revoked) commitments for this week
    const { data: activeCommitments, error: activeCommitmentsErr } = await supabase
      .from("commitments")
      .select("id, user_id, week_start_date, week_end_date, limit_minutes, penalty_per_minute_cents, monitoring_status")
      .eq("week_end_date", deadlineStr)
      .neq("monitoring_status", "revoked")
      .in("status", ["pending", "active"]);
    
    if (activeCommitmentsErr) {
      console.error("Error fetching active commitments for no-sync check:", activeCommitmentsErr);
    } else {
      for (const c of activeCommitments ?? []) {
        // Check if this commitment has ANY daily_usage entries for this week
        const { data: existingUsage, error: usageCheckErr } = await supabase
          .from("daily_usage")
          .select("id")
        ...
        if (usageCheckErr) {
          console.error(`Error checking daily_usage for commitment ${c.id}:`, usageCheckErr);
          continue;
        }
        
        // If no usage data exists, apply worst-case estimation
        if (!existingUsage || existingUsage.length === 0) {
          console.log(`No sync data found for commitment ${c.id} (user ${c.user_id}), applying worst-case estimation`);
          
          // Calculate week start and end dates
          const weekStart = new Date(c.week_start_date || deadlineStr);
          const weekEnd = new Date(c.week_end_date || deadlineStr);
          
          // Insert worst-case usage for each day of the week
          // Worst-case: Assume double the limit (same as revoked monitoring)
          ...
          while (d <= weekEnd) {
            ...
            // Insert worst-case estimate (only if not already exists)
            const { error: insertErr } = await supabase.from("daily_usage").insert({
              ...
              penalty_cents: penaltyCents,
              is_estimated: true,
              source: "no_sync_worst_case",
              reported_at: new Date().toISOString()
            });
            
            if (insertErr && !insertErr.message.includes("duplicate")) {
              console.error(`Error inserting worst-case daily_usage for ${dayStr}:`, insertErr);
            }
