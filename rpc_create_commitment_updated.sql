...
-- Drop by type signature (order matters in PostgreSQL)
DROP FUNCTION IF EXISTS public.rpc_create_commitment(date, integer, integer, jsonb);
-- Also try dropping with old parameter names if they exist
DROP FUNCTION IF EXISTS public.rpc_create_commitment(
...
-- Note: p_deadline_date is the deadline (next Monday before noon), not the start date
-- The commitment starts NOW (current_date) when the user commits
